...
public class DifficultyPool {
	...
	static BeatmapSingerBot CURTAIN_CALL_SS = new BeatmapSingerBot(new ArrayList<Interval>(Arrays.asList(
			...
			new Interval(12000, 4))), FilePaths.CURTAIN_CALL, "Curtain Call", "SS");
	
	static VSingerBot VS_A = new VSingerBot(4, 10000, 40);
	
	static FriendlySingerBot SPRINT = new FriendlySingerBot();
	static List<SingerBot> DEFAULT_BEATMAP_BOTS = new ArrayList<SingerBot>(Arrays.asList(CURTAIN_CALL_SS));
	static List<SingerBot> DEFAULT_VS_BOTS = new ArrayList<SingerBot>(Arrays.asList(VS_A));
	static List<SingerBot> DEFAULT_BOTS = new ArrayList<SingerBot>(Arrays.asList(CURTAIN_CALL_SS, VS_A));
}
